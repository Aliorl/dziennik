...
<html lang="en">
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Lista tematów</h1>
                        <div class="card-header"><img src="gfx/015-tick.svg" style="width:4%;"><b>Dodawanie tematów i edycja danych:</b>
                        <p class="mb-0">
                           <form action="tematy.php" method="post">
                                <?php if (isset($_SESSION['username'])){?>
                                	<form action="tematy.php" method="post">
                                	<input type='hidden' name="id" value="<?=$ID?>">
                                	Wprowadź temat: <input type='text' name="temat" placeholder="temat" title="Temat" value="<?=$TEMAT?>">
                                	<br>Wybierz prowadzącego: <select id="id_prowadzacego" name="id_prowadzacego" title="prowadzacy">
								<?php
								foreach ($PROWADZACY as $prowadzacy){
									...
									<br>Wybierz zajęcia: <select id="id_zajec" name="id_zajec" title="Zajecia">
								<?php 
							...
							<div class="card-body">
								<div class="w3-bar">
  									<button class="w3-bar-item w3-button w3-round w3-green" style="width:30%" type="submit" name="dodaj">Dodaj nowy temat</button>
	...
	<?php } ?>
                        <form action="tematy.php" method="post">
                        <div class="card-header"> Filtruj po wybranych zajęciach: 
                        	<select id="nazwa_zajec" name="nazwa_zajec" title="Zajecia">
                        		<?php
                        	       foreach ($ZAJECIA as $zajecia){
	                                   $selected=($ID_ZAJEC==$zajecia["id_zajec"])?" selected ":"";
	                                   echo '<option value="'.$zajecia["nazwa_zajec"].'"'.$selected.'>'.$zajecia["nazwa_zajec"].'</option>'.PHP_EOL;
	                           }?>
							</select>
							<button class="w3-bar-item w3-button w3-round w3-dark-grey" style="width:20%"type="submit" name="wyświetl">Wyświetl listę</button>
						</div>
						<div class="card-header"><strong><?php if (isset($_POST['nazwa_zajec'])) echo '<i class="fas fa-columns"></i> Lista tematów do wybranych zajęć: '.$_POST['nazwa_zajec']?></strong></div>
						</form>
<!--                         </div> -->
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                echo '<tr><th>Temat</th><th>Prowadzący</th><th>Zajęcia</th>'.$dodatkowa_kolumna.'</tr>'.PHP_EOL;
                                foreach ($TEMATY as $temat){
                                    echo "<tr>";
                                    echo "<td>".$temat["temat"]."</td>
                                    <td>".$temat["stopien_naukowy"]." ".$temat["imie"]." ".$temat["nazwisko"]."</td>
                                    <td>".$temat["nazwa_zajec"]."</td>";
                                    if (isset($_SESSION['username'])){
                                        echo "<td>";
                                        echo "<a class='btn btn-primary btn-block' title='Edytuj' href='tematy.php?op=0&id=".$temat['id_tematu']."'>Edytuj</a>";
                                        echo "</td>";
                                        echo "<td>";
                                        echo "<a class='w3-button w3-red' title='Usuń' href='tematy.php?op=1&id=".$temat['id_tematu']."'>Usuń</a>";
                                        echo "</td>";
                                    }
                                    echo "</tr>".PHP_EOL;
                                }
